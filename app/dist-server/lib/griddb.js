import { GridDBError } from './types/griddb.types.js';
/**
...
export function createGridDBClient(config) {
    ...
    async function makeRequest(path, payload, method) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new GridDBError(`HTTP error! status: ${response.status} - ${responseText || response.statusText}`, response.status, response.status, responseText);
            }
    ...
    async function createContainer({ containerName = 'sentiments', columns = [
    ...
    async function insertData({ data, containerName = 'sentiments' }) {
        console.log(data);
        try {
            const row = [
